...
async def last_matchs(session, today, match_vs, admin_url, header, player_id, match_id, last_flag):
    tour_now_html = await fetch_data(session, admin_url, {"action": "lazyLoadPanes", "pane": "activity", "matchID": match_id})
    tour_now = BeautifulSoup(tour_now_html, "lxml").find(id="basic-this-tournament-A")["value"].split("At ")[1]
    ...
    for i in range(2, len(show_class)):
        ...
        if tour_now in data.find(id="activity-table").find("a").text:
            stats_match_html = await fetch_data(session, admin_url, {"action": pAB_matchs[m_i].find_all("a")[1]["data-action"], "matchID": pAB_matchs[m_i].find_all("a")[1]["data-matchid"]})
